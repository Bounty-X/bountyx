...
export function FormSelect(props: FormSelectProps) {
  ...
  return (
    <Field name={fieldName}>
      {({ field, meta }: FieldProps) => (
        <FormControl sx={{ m: 1, minWidth: 120 }} className={className} error={meta.touched && !!meta.error} disabled={disabled}>
          ...
          <Select
            ...
            label={label}
            multiple={multiple}
            renderValue={(selected: string[] | string) => (_.isArray(selected) ? selected.join(', ') : selected)}>
...
export function FormDatePicker(props: FormDatePickerProps) {
  const { className, fieldName, label, showUndefined, defaultUndefined, disabled } = props
  ...
  const hasError = formikProps && formikProps.touched[fieldName] && !!formikProps.errors[fieldName]
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      ...
      <FormControl error={hasError}>
        ...
        />
        {
          ...
          hasError && dateUndefined ? <FormHelperText>{errorMessage}</FormHelperText> : <></>
        }
      </FormControl>
...
export function FormCheckbox(props: FormCheckboxProps) {
  ...
  return (
    <Field name={fieldName} className={className}>
      {({ field }: FieldProps) => <Checkbox {...field} defaultChecked={defaultChecked} disabled={disabled} />}
    </Field>
  )
}
