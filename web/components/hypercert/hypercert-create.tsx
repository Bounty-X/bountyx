...
import { useMintClaimAllowlist } from '../../hooks/hypercert/mintClaimAllowlist'
...
const formDataToQueryString = (values: Record<string, any>) => {
  ...
  ;['impactTimeStart', 'impactTimeEnd', 'workTimeStart', 'workTimeEnd'].forEach(formatDate)
  const filteredValues = _.chain(values).pickBy().omit(['agreeContributorsConsent', 'agreeTermsConditions']).value()
...
const ValidationSchema = Yup.object().shape({
  ...
  description: Yup.string()
    ...
    .required('Required'),
  externalLink: Yup.string().test('valid uri', 'Please enter a valid URL', (value) =>
    isValidUrl(value, {
      emptyAllowed: true,
      ipfsAllowed: true,
    })
  ...
  allowlistUrl: Yup.string().test('valid uri', 'Please enter a valid URL', (value) =>
    isValidUrl(value, {
      emptyAllowed: true,
      ipfsAllowed: false,
    })
  ),
  agreeContributorsConsent: Yup.boolean().oneOf([true], 'You must get the consent of contributors before creating'),
